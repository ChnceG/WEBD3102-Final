...
export default function AddIncome() {
    ...
    const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
        ...
        const selectedCategoryObj = categories.find(category => category.categoryId);
        const selectedAccountObj = accounts.find(account => account.accountId);
        ...
        axios.post(`/api/income?accountId=${incomeData.account.accountId}`, incomeData)
            .then(() => {
    ...
    return (
        <InfoBox className="w-[600px] mx-auto p-2 bg-white">
            <div className="flex flex-col">
                <h2 className="text-lg font-bold bg-white text-center">Income Page</h2>
                <form onSubmit={handleSubmit} className="bg-white flex flex-col gap-2">
                    ...
                    <div className="flex flex-col items-center bg-white">
                        <button type="submit" className='bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded'>Add Income</button>
                    </div>
                </form>
            </div>
        </InfoBox>
